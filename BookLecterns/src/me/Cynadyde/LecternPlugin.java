...
public class LecternPlugin extends JavaPlugin implements Listener {
	...
	public static final String chatTag = 
			...
			"Lecturns" + ChatColor.DARK_GRAY + "]" + ChatColor.RESET + " ";
	...
	public void onPlayerInteract(PlayerInteractEvent event) {
		...
		LecternBlock lecturn = LecternBlock.get(event.getClickedBlock());
		if (lecturn == null) {
			return;
		}
		...
		if (lecturn.getContents().size() == 1 && quickDisplay) {
			openBook(event.getPlayer(), lecturn.getContents().get(0));
			return;
		}
		else {
			LecternGUI.addViewer(event.getPlayer(), lecturn);
			return;
		}
	}
		...
		if (event.getClickedInventory() != null) {
			if (event.getClickedInventory().getTitle().startsWith(LecternGUI.titlePrefix)) {
				if (event.getWhoClicked() instanceof Player) {
					...
					// User clicked the forewards navigation button in the GUI...
