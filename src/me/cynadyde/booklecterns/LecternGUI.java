...
/**
 * Display the contents of a lectern to a player through an inventory GUI.
 */
...
public class LecternGUI {
    ...
    public static final int backButtonIndex = (slotsPerRow) + 1;
    public static final int nextButtonIndex = (slotsPerRow) + 7;
    ...
    private LecternGUI(Player player, LecternBlock lectern) {
        ...
        String title = titlePrefix + ((lectern.getDesc().equals("")) ? "no description" : lectern.getDesc());
    ...
    public void updateDisplay(int change) {
        ...
        List<ItemStack> viewedContents = (endIndex > startIndex) ? lectern.getContents().subList(startIndex, endIndex) : new ArrayList<>();
        ...
        for (int i = 0; i < 5; i++) {
            inventory.setItem((slotsPerRow) + 2 + i, (i > viewedContents.size() - 1) ?
                    background[(slotsPerRow) + 2 + i] : viewedContents.get(i));
        }
